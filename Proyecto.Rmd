...
Para realizar un análisis de expresión diferencial, es primordial eliminar los datos innecesarios y no favorecer las muestras con mayor profundidad de secuenciación. Sin embargo, la eliminación de estos datos depende de las condiciones que utilizamos para cada experimento. Por ejemplo, a partir del archivo con los datos de expresión: all_tally_counts.tab.gz,  al modificar los umbrales de expresión, observamos que tantos datos estamos eliminando de nuestro análisis. Decidimos modificar los umbrales de expresión de 1 a 2, obteniendo que la cantidad de datos eliminados se modifica en aproximadamente un 50% :

```{r}
#Cargamos Arch
countFile <- "C:\\Users\\Alberto\\Dropbox\\LCG\\Sem 4\\BIF_II\\Mod_1\\Proyecto\\E_diferencial\\all_tally_counts.tab.gz"
#leemos como tabla
...
Al explorar los datos nos pudimos percatar que las réplicas se mantienen agrupadas, a excepción de las muestras que contenían RNA degradado, asimismo los datos que más se separan en el eje de las x´s son aquellos que provienen de las vesículas purificadas. A pesar de observar lo anterior, se podría considerar trabajar con estos datos. 
...
Al observar los gráficos de los datos normalizados, nos percatamos que los datos provenientes de las vesiculas sesgaron  la normalización del resto de los datos.Este fenómeno se debe a que los datos de las vesículas difieren mucho de los datos de las líneas celulares. En los gráficos se visualiza que el conjunto de puntos de cada muestra no estan centrados con respecto a la línea roja.

Asimismo podemos observar que al realizar la normalización, los datos de las células control y de las células tratadas tienden a agruparse sobre el eje de las x's , mientras que los datos de las vesículas purificadas se mantienen alejados.
...
Posteriormente, se realizó nuevamente la normalización de los datos, eliminando las muestras de las vesículas purifcadas y la muestra degradada.Este criterio mejoró notablemente la normalización de los datos, ya que se pudo observar que el conjunto de puntos se encuentra centrado respecto a la línea roja.Asimismo, un aspecto a resaltar de los datos de las células tratadas es la presencia de un pico en el cuadrante superior del gráfico, el anterior puede indicar la presencia de secuencias provenientes de las vesículas.
...
Designamos a que corresponde cada condición,y preparamos una matriz; de esta manera podíamos calcular parámetros de sobre-dispersión, como sabemos esta última puede ser calculada a tres niveles: Common, para el valor representativo de todos los genes;Trended, para los rangos de niveles de expresión y Tagwise, para el valor individual para cada gen.

Lo que realizamos a continuación fue probar las diferentes dispersiones y observar la estimación de los genes con baja y alta expresión. Encontramos que la dispersión es dependiente entre los genes y la relación de varianza entre los mismos es diferente. Con la estimación "Common" observamos cuál es la dispersión común para el conjunto de nuestros datos, es decir pudimos tener noción de la variabilidad de los mismos; lo cual es de vital importancia en casos como estos donde tenemos réplicas. 

Por otra parte, en el caso de la dispersión "tagwise" pudimos determinar la dispersión para cada transcrito. En los gráficos, la dispersión de los datos nos indica la variabilidad entre las réplicas; pero además, la dispersión que presenta cada gen nos esta hablando de su abundancia, pues definimos cuántos genes estan cumpliendo con los criterios que nosotros seleccionamos (activados o reprimidos con un FDR <5%).
...
topTags(lrt)
dt = decideTestsDGE(lrt, adjust.method="BH", p.value=0.05, lfc=0)
...
```{r}
cosa<-topTags(lrt)
subset(cosa, cosa$table$FDR>.05)
```
```{r}
cosa$table$FDR[cosa$table$FDR>0.5]
```
